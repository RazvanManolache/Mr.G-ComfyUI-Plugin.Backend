...
class Workflows(NamedObject):
    ...
    queue_runs = relationship('QueueRuns', back_populates='workflow')
    

# QueueRuns
class QueueRuns(BaseModel):
    ...
    workflow = relationship('Workflows', back_populates='queue_runs')
    api = relationship('Api', back_populates='queue_runs')
    job = relationship('Jobs', back_populates='queue_runs')
    queue_steps = relationship('QueueSteps', back_populates='queued_run')

# QueueSteps
class QueueSteps(BaseModel):
    __tablename__ = 'queue_steps'
    queued_run_uuid = Column(String, ForeignKey('queue_runs.uuid'), nullable=True)
    ...
    update_date = Column(DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow)
    end_date = Column(DateTime, nullable=True)
    queued_run = relationship('QueueRuns', back_populates='queue_steps')
...
class Jobs(WorkflowExtenders): #js model
    ...
    queue_runs = relationship('QueueRuns', back_populates='job')
...
class Api(WorkflowExtenders): #js model
    ...
    queue_runs = relationship('QueueRuns', back_populates='api')
...
def insert_queue_run(data):
    return session.add(QueueRuns(**data))

def update_queue_step(data):
    return session.merge(data)

def get_queue_run_status_for_uuid_list(uuids):
    return session.query(QueueRuns).filter(QueueRuns.uuid.in_(uuids)).all()

def get_all_outputs_for_run(uuid):
    return session.query(Outputs).join(QueueSteps).join(QueueRuns).filter(QueueRuns.uuid==uuid).all()
...
def update_queue_run(data):
    return session.merge(data)

def get_queued_run_by_id(uuid):
    return session.query(QueueRuns).get(uuid)

def get_queue_runs_by_statuses(statuses):
    return session.query(QueueRuns).filter(QueueRuns.status.in_(statuses)).all()

def insert_queue_step(data):
    return session.add(QueueSteps(**data))
 
def get_queue_runs_max_order():
    rec = session.query(QueueRuns).order_by(QueueRuns.order.desc()).first()
    if rec:
        return rec.order
    return 0
